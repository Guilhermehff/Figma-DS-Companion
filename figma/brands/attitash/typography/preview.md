...
Review state: written in Figma. Verify live state in `figma/brands/attitash/brand.yml`.

## Source Roles

- `Headline` -> `Buenos Aires Bold`, title case, `5px` tracking
- `Body` -> `Buenos Aires Light`, `5px` tracking
- `CTA` -> `Buenos Aires Bold`, all caps, `5px` tracking
- `Subhead` -> `Coordinates Medium`, all caps, `100px` tracking
